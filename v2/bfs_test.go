...
func TestTravelBfs(t *testing.T) {
	...
	traversal := StackBFSTraversal[string]{}
	...
	assert.Equal(t, "Pekanbaru", path[2].GetValue())
	assert.Equal(t, "Padang", path[3].GetValue())
	assert.Equal(t, "Batam", path[4].GetValue())
	assert.Equal(t, "Jambi", path[5].GetValue())
	assert.Equal(t, "Bengkulu", path[6].GetValue())
	assert.Equal(t, "Palembang", path[7].GetValue())
	assert.Equal(t, "Banda Lampung", path[8].GetValue())
	assert.Equal(t, "Pangkal Pinang", path[9].GetValue())
}
